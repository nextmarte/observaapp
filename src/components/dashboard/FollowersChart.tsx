...
export const FollowersChart: React.FC<FollowersChartProps> = ({
  data,
  loading = false,
}) => {
  ...
  const { chartData, profiles } = useMemo(() => {
    ...
    const latestValidByProfile = new Map<string, number>();
    ...
    data.forEach((item) => {
      ...
      const followersValue =
        typeof item.seguidores === 'number' && item.seguidores > 0
          ? item.seguidores
          : null;
      weekData[item.perfil_id] = followersValue;

      if (typeof item.seguidores === 'number' && item.seguidores > 0) {
        latestValidByProfile.set(item.perfil_id, item.seguidores);
      }
    });
    ...
    const visibleProfiles = sortedProfiles
      .sort((a, b) => Number(latestValidByProfile.get(b.id) || 0) - Number(latestValidByProfile.get(a.id) || 0))
      .slice(0, 5);
  ...
  return (
    <Card className="bg-card border-border">
      <CardHeader>
        ...
        <p className="text-xs text-muted-foreground">Top 5 perfis por base atual</p>
      </CardHeader>
      <CardContent>
